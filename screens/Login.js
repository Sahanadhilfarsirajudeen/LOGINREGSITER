...
const Login = ({ navigation }) => {
  ...
  const handleLogin = async () => {
    try {
      ...
      const response = await axios.post('http://192.168.1.104:3000/auth/login', userData);
  ...
  const handleForgotPassword = () => {
    navigation.navigate('PasswordChange'); // Navigate to PasswordChange page
  };
    ...
    <SafeAreaView style={{ flex: 1, backgroundColor: COLORS.white }}>
      <View style={{ flex: 1, marginHorizontal: 22 }}>
        ...
        <TouchableOpacity onPress={handleForgotPassword}>
          <Text style={{ marginTop: 10, color: COLORS.primary }}>Forgot Password</Text>
        </TouchableOpacity>
      </View>
    </SafeAreaView>
  );
